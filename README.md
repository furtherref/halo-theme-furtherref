...
FurtherRef 的 Halo 2.0 主题。

![FurtherRef](./screenshot.jpg)

## 使用方式

1. 手动从以下地址下载主题包并在 Console 的主题管理界面安装，安装方式可参考：<https://docs.halo.run/user-guide/themes>
   - https://github.com/furtherref/halo-theme-furtherref/releases

2. 如果安装了[应用市场](https://www.halo.run/store/apps/app-VYJbF)插件，可以直接在应用市场中搜索`FurtherRef`并安装。

## 插件支持

FurtherRef 主题支持以下 Halo 插件：
...
```bash
git clone git@github.com:furtherref/halo-theme-furtherref.git ~/halo2-dev/themes/theme-furtherref
```

```bash
cd ~/halo2-dev/themes/theme-furtherref
```
